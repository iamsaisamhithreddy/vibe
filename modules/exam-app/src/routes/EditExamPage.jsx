...
function ExamSettings({ exam }) {
  ...
  const save = () => {
    examStore.update(exam.id, { title, duration, passingMarks, negativeMarking, instructions })
    setSaved(true)
...
function QuestionsSection({ exam }) {
  return (
    <section>
...
function QuestionRow({ examId, question, index }) {
...
function AddQuestionForm({ examId }) {
  ...
  return (
    <div className="rounded-md border border-border bg-card p-4 shadow-sm">
      ...
      <QuestionForm
        ...
        onSubmit={(data) => {
          examStore.addQuestion(examId, data)
          setOpen(false)
        }}
      />
    </div>
